...
# Waste by Category
st.header("Waste by Food Category")
by_category = (df.groupby('food_category', as_index=False)['total_waste_(tons)']
                 .sum()
...
if not forecast_df.empty and 'ARIMA_forecast' in forecast_df.columns and 'date' in forecast_df.columns:
    fig, ax = plt.subplots(figsize=(10,5))
    ...
    st.warning("Forecast data is empty or missing required columns!")

# Country Clusters
st.header("Country Clusters")
if not clusters_df.empty:
    fig, ax = plt.subplots(figsize=(10,5))
    sns.scatterplot(data=clusters_df, x='total_waste', y='economic_loss', hue='cluster',
                    palette="deep", s=100, ax=ax)
